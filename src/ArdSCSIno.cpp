...
void setup()
{
  ...
  // SASI commands that have handlers
  scsi_command_table[SASI_ASSIGN] = onAssign;
  scsi_command_table[SASI_SPECIFY] = onSpecify;
  ...
  memcpy(&default_hdd.product, "ArdSCSino", 9);
  ...
  memcpy(&default_optical.vendor, "ArdSCSino", 9);
...
void initFileLog(int success_mhz) {
  ...
  LOG_FILE.println("ArdSCSino <-> SD - https://github.com/ztto/ArdSCSino-stm32");
...
//  LOGN("onRead6"); LOG("-R "); LOGHEX(adds); LOG(":"); LOGHEXN(len);
...
//  LOGN("onRead10"); LOG("-R "); LOGHEX(adds); LOG(":"); LOGHEXN(len);
  byte sts = checkBlockCommand(dev, adds, len);
  if (sts) {
    return sts;
  }
...
static byte onWrite6(SCSI_DEVICE *dev, const byte *cdb)
{
  unsigned adds = (((uint32_t)cdb[1] & 0x1F) << 16) | ((uint32_t)cdb[2] << 8) | cdb[3];
  unsigned len = (cdb[4] == 0) ? 0x100 : cdb[4];
//  LOGN("onWrite6"); LOG("-W "); LOGHEX(adds); LOG(":"); LOGHEXN(len);
...
static byte onWrite10(SCSI_DEVICE *dev, const byte *cdb)
{
  unsigned adds = ((uint32_t)cdb[2] << 24) | ((uint32_t)cdb[3] << 16) | ((uint32_t)cdb[4] << 8) | cdb[5];
  unsigned len = ((uint32_t)cdb[7] << 8) | cdb[8];
//  LOGN("onWrite10"); LOG("-W "); LOGHEX(adds); LOG(":"); LOGHEXN(len);
  if(dev->m_type == SCSI_DEVICE_OPTICAL)
  {
    dev->m_senseKey = SCSI_SENSE_HARDWARE_ERROR;
    dev->m_additional_sense_code = SCSI_ASC_WRITE_PROTECTED; // Write Protect
...
/*
 * ASSIGN Command(SASI) processing.
 */
static byte onAssign(SCSI_DEVICE *dev, const byte *cdb)
{
  LOGN("ASSIGN");
  byte a[5];
  memset(a, 0, sizeof(a));
...
//  if(SCSI_IN(vIO) || (scsiid) == 0) {
  ...
  WAIT(m_X68SasiMsec); LOG(":"); LOGHEX(cmd[0]);
  // Command length selection, reception
  static const int cmd_class_len[8]={6,10,10,6,6,12,6,6};
  len = cmd_class_len[cmd[0] >> 5];
  cmd[1] = readHandshake(); WAIT(m_X68SasiMsec); LOG(":"); LOGHEX(cmd[1]);
  cmd[2] = readHandshake(); WAIT(m_X68SasiMsec); LOG(":"); LOGHEX(cmd[2]);
  cmd[3] = readHandshake(); WAIT(m_X68SasiMsec); LOG(":"); LOGHEX(cmd[3]);
  cmd[4] = readHandshake(); WAIT(m_X68SasiMsec); LOG(":"); LOGHEX(cmd[4]);
  cmd[5] = readHandshake(); WAIT(m_X68SasiMsec); LOG(":"); LOGHEX(cmd[5]);
...
//  cmd[5] = readHandshake(); LOG(":"); LOGHEX(cmd[5]);
  ...
  for(int i = 6; i < len; i++ ) {
    cmd[i] = readHandshake(); WAIT(m_X68SasiMsec); LOG(":"); LOGHEX(cmd[i]);
//    cmd[i] = readHandshake(); LOG(":"); LOGHEX(cmd[i]);
  }
  // LUN confirmation
